...
public class SlashCommandsConfiguration {
    ...
    public static void configureSlashCommands(DIApi api) {
        api.getCoreController().getBot().getApi().get().updateCommands().addCommands(
                ...
                Commands.slash(CommandAliasController.getAlias("prejoin_confirm_command").toLowerCase(), "Confirm prejoin verification with the code from your kick screen")
                        .addOption(OptionType.STRING, "code", "Code shown when you were kicked from the server", true, false)
        ).queue();
    }
}
